...
class SpeakerTestUseCases:
    ...
    def run_speaker_test(
        ...
        self._log_step(f"check test device MicIn + {test_device_output_type} capability")
        ...
        try:
            ...
            self._log_step(f"configure test device to MicIn + {test_device_output_type} + max volume")
            two_way_audio_status = self.isapi.ensure_two_way_audio_channel_ready(
                session=session,
                require_input_type="MicIn",
            ...
            if not self._input_type_supported(two_way_audio_status, "MicIn"):
                raise HikvisionSDKError(
            ...
            if not self._output_type_supported(two_way_audio_status, test_device_output_type):
                raise HikvisionSDKError(
                    f"test device does not support {test_device_output_type} output",
                    api_name=two_way_audio_status.capability_path or "GET /ISAPI/System/TwoWayAudio/channels/capabilities",
                )
            if two_way_audio_status.input_type != "MicIn":
                raise HikvisionSDKError(
                    "test device failed to switch audioInputType to MicIn",
                    api_name=two_way_audio_status.config_path or "PUT /ISAPI/System/TwoWayAudio/channels/<channel>",
                    error_message=f"current_input_type={two_way_audio_status.input_type}",
                )
            if two_way_audio_status.output_type != test_device_output_type:
                raise HikvisionSDKError(
                    f"test device failed to switch audioOutputType to {test_device_output_type}",
                    api_name=two_way_audio_status.config_path or "PUT /ISAPI/System/TwoWayAudio/channels/<channel>",
                    error_message=f"current_output_type={two_way_audio_status.output_type}",
                )
            if audio_compression_type and not self._audio_compression_type_supported(two_way_audio_status, audio_compression_type):
                raise HikvisionSDKError(
                    f"test device does not support audioCompressionType={audio_compression_type}",
                    api_name=two_way_audio_status.capability_path or "GET /ISAPI/System/TwoWayAudio/channels/capabilities",
                )
            if audio_compression_type and two_way_audio_status.audio_compression_type != audio_compression_type:
                raise HikvisionSDKError(
                    f"test device failed to switch audioCompressionType to {audio_compression_type}",
                    api_name=two_way_audio_status.config_path or "PUT /ISAPI/System/TwoWayAudio/channels/<channel>",
                    error_message=f"current_audio_compression_type={two_way_audio_status.audio_compression_type}",
                )
            self._log_step(
                "test device ready "
